    ...
    if (iOS8Later)
    {
        ...
        for (PHCollection  *collection in collectonResuts)
        {
            if ([collection.localizedTitle isEqualToString:folderName])  // folderName是我们写入照片的相册
            {
                ...
                if (completion) completion(collection,nil,isExisted);
                break;
            }
        }
        if (!isExisted)
    ...
    [assetsLibrary enumerateGroupsWithTypes:ALAssetsGroupAlbum usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
        ...
        else
        {
            ...
            // 系统允许存在同名的相册多个,回调以便利的最后一个作为返回值
             if (completion) completion(group,nil,isExisted);
        }
        ...
        if (iOS8Later)
        {
            if ([self isExistFolder:albumName completion:nil])  // 已经存在
            {
                ...
                if ([self authorizationStatusAuthorized]) // 已经授权
                {
                    ...
                    // 创建相册成功,并返回
                    if (completion) completion( [PHAssetCollection fetchAssetCollectionsWithLocalIdentifiers:@[photoID] options:nil].firstObject,error,error? NO : YES);
                }else{   // 没有授权直接打开
            ...
            [assetsLibrary enumerateGroupsWithTypes:ALAssetsGroupAlbum usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
                ...
                else
                {
                    ...
                    for (ALAssetsGroup *gp in groups)
                    {
                        ...
                        {
                            haveHDRGroup = YES;
                            if (completion) completion(group,nil,YES);
                            break;
                        }
                    }
                    if (!haveHDRGroup)
...
- (BOOL) saveImageIntoAlbumFromImage:(id)image albumName:(NSString *)albumName completion:(void (^)(NSError *error,BOOL isExistedOrIsSuccess))completion
{
    ...
    if (iOS8Later)
    {
        ...
        if ([self isExistFolder:albumName completion:nil]) // 已经存在直接保存
        {
            ...
            if (completion) completion(error,YES);
              if (!error) return YES;
        }else{  // 创建相册
            ...
            if (completion) completion(error,error? NO : YES);
            if (!error) return YES;
        }
    }else{
...
- (BOOL)saveVideoIntoAlbumFromVideo:(id)video albumName:(NSString *)albumName completion:(void (^)(NSError *error,BOOL isExistedOrIsSuccess))completion
{
    if (iOS8Later)
    {
        if (![video isKindOfClass:[NSURL class]]) return NO;
...
- (id) saveImageIntoSystemAlbumFromImage:(id)resources completion:(void (^)(id assetResult,NSError *error,BOOL isExistedOrIsSuccess))completion
{
    ...
    if (iOS8Later)
    {
        ...
        if (completion) completion([PHAsset fetchAssetsWithLocalIdentifiers:@[assetID] options:nil],error,error? NO : YES);
        if (error) return nil;
        ...
        [weakLibrary writeImageToSavedPhotosAlbum:resourceImage.CGImage metadata:nil completionBlock:^(NSURL *assetURL, NSError *error) {
            if (completion) completion(assetURL,error,error? NO : YES);
        }];
    }
        return nil;
}
...
- (id)saveVideoIntoSystemAlbumFromVideoUrl:(NSURL *)videoUrl completion:(void (^)(id assetResult,NSError *error,BOOL isExistedOrIsSuccess))completion
{
    ...
    if (iOS8Later)
    {
        ...
        if (completion) completion([PHAsset fetchAssetsWithLocalIdentifiers:@[assetID] options:nil],error,YES);
        if (error) return nil;
        return  [PHAsset fetchAssetsWithLocalIdentifiers:@[assetID] options:nil];
    }else{
    // ios 8 之前
...
-(void)deleteAssetArr:(NSArray *)assetArr completion:(completionHandler)completion
{
    if (iOS8Later)
    {
        ...
        {
            if (![assetArr[i] isKindOfClass:[PHAsset class]]) return;
        }
    ...
    else
    {
        for (int i = 0 ; i < assetArr.count; i++)
        {
            if (![assetArr[i] isKindOfClass:[ALAsset class]]) return;
        }
...
- (void)_requestAuthorizationWithCompletion:(void (^)())completion
{
...
- (void)_startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset completion:(void (^)(NSString *outputPath))completion {
...
// ios 7 删除图片扩展
@implementation IJSAssetAPIManager
...
@implementation ALAsset (IJSAssetAPIManager)
